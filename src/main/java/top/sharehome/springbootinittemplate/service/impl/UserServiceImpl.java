...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public Page<AdminUserPageVo> adminPageUser(AdminUserPageDto adminUserPageDto, PageModel pageModel) {
        ...
        userLambdaQueryWrapper
                ...
                .like(StringUtils.isNotBlank(adminUserPageDto.getAccount()), User::getAccount,
                        adminUserPageDto.getAccount())
    ...
    public void adminImportUser(MultipartFile file) {
        try {
            List<AdminUserImportDto> userImportDtoList = ExcelUtils.importStreamSyncAndClose(file.getInputStream(),
                    AdminUserImportDto.class);
            List<User> list = userImportDtoList.stream().map(user -> {
                ...
                if (!user.getAccount().matches("^[A-Za-z0-9]{2,16}$")) {
                    throw new CustomizeReturnException(ReturnCode.EXCEL_FILE_ERROR,
                            "存在账号[" + user.getAccount() + "]格式错误，要求介于2-16位且不包含特殊字符");
                }
                ...
                if (!user.getName().matches("^.{1,16}$")) {
                    throw new CustomizeReturnException(ReturnCode.EXCEL_FILE_ERROR,
                            "存在名称[" + user.getName() + "]格式错误，要求介于1-16位");
                }
                LambdaQueryWrapper<User> userLambdaQueryWrapper = new LambdaQueryWrapper<>();
                userLambdaQueryWrapper
